...
function NodeListItem({
  ...
  return (
    ...
    >
      <span
        className={`h-2.5 w-2.5 shrink-0 ${tier === "support" ? "rounded-[3px]" : "rounded-full"}`}
        style={
          tier === "support"
...
function ValueGroup({ value, selectedId, onSelect, query, tierFilter }: ValueGroupProps) {
  ...
  const showGroup = query
    ...
    : tierFilter === "all" || anyRootVisible || matches(value.title);
          ...
          <div key={r.id}>
            ...
            {r.supports.map((s) => (
              <NodeListItem
                ...
                onSelect={onSelect}
                visible={(tierFilter === "all" || tierFilter === "support") && (query ? matches(s.title) : true)}
                indent={52}
              />
            ))}
          </div>
  ...
  return (
    <div className="flex h-full min-h-[320px] flex-col items-center justify-center gap-4 p-10 text-center">
      ...
      <div>
        ...
        <p className="mx-auto mt-1.5 max-w-[300px] text-[12.5px] leading-relaxed text-mist-500">
          Редактируйте обещания — изменения сразу попадают на дерево.
        </p>
      </div>
    </div>
  );
...
function NodeEditor({ node, onSave, onUpdateCompany }: NodeEditorProps) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    savedTimer.current = window.setTimeout(() => setSaved(false), SAVED_INDICATOR_MS);
  };
  ...
  return (
    <form onSubmit={submit} className="mx-auto w-full max-w-[640px] p-5 md:p-8">
      ...
      <h2 className="font-display mt-4 text-[19px] leading-snug font-bold text-mist-100">
      ...
      <div className="mt-6 space-y-5">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                disabled={uploading}
                className="rounded-lg border border-ink-700/60 bg-ink-850 px-4 py-2 text-[12px] font-semibold text-mist-300 transition hover:border-lagoon/50 hover:text-lagoon disabled:cursor-not-allowed disabled:opacity-50"
              >
        ...
        <div>
          ...
          <input
            ...
            placeholder={isCompany ? "Например: Acme Corp" : "Текст обещания"}
          />
        </div>
        ...
        {hasShort && (
          <div>
            <label className="field-label">
              {isCompany ? "Надпись под кружком" : "Короткая подпись"}{" "}
              <span className="normal-case tracking-normal opacity-60">
                — {isCompany ? "под центральным кружком" : "на кружке дерева"}
              </span>
            </label>
            <input
              className="field"
              ...
              placeholder={isCompany ? "Например: КОМПАНИЯ" : "Кратко, 20–25 символов"}
            />
          </div>
        )}
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={isCompany ? "Опишите компанию и её миссию" : "Раскройте смысл обещания"}
          />
        </div>
      ...
      <div className="mt-7 flex items-center gap-3">
        <button
          type="submit"
          disabled={!dirty}
          className={`font-display rounded-lg px-6 py-2.5 text-[13px] font-bold tracking-wide transition-all duration-200 ${
            dirty
              ? "bg-gold text-ink-950 hover:-translate-y-0.5 hover:brightness-110 hover:shadow-lg hover:shadow-gold/25 active:translate-y-0"
...
function ImportSection({ onApplied }: ImportSectionProps) {
  ...
  return (
    <div className="panel-scroll min-h-0 flex-1 overflow-y-auto">
      <div className="mx-auto w-full max-w-[760px] p-5 md:p-8">
        ...
        <p className="mt-1.5 max-w-[560px] text-[13px] leading-relaxed text-mist-400">
          Загрузите файл с обещаниями — колонки распознаются по названиям, дерево пересоберётся автоматически.
        </p>
        ...
        {result && (
          <div className="mt-5 overflow-hidden rounded-2xl border border-lagoon/35 bg-ink-900/70">
            ...
            <div className="flex flex-wrap items-center gap-x-7 gap-y-2 px-5 py-4">
              {[
                { n: result.stats.rows, label: "строк", c: "#93abb2" },
                { n: result.stats.values, label: "ценностей", c: "#f2b45a" },
                { n: result.stats.roots, label: "корневых", c: "#43d6b5" },
            ...
            <div className="flex flex-wrap items-center gap-3 border-t border-ink-700/60 bg-ink-950/40 px-5 py-4">
              <button
                ...
                className="font-display rounded-lg bg-lagoon px-6 py-2.5 text-[13px] font-bold text-ink-950 transition-all hover:-translate-y-0.5 hover:brightness-110 hover:shadow-lg hover:shadow-lagoon/25 active:translate-y-0"
              >
                Применить импорт
              </button>
              <button
                onClick={() => setResult(null)}
                className="rounded-lg border border-ink-700/70 px-4 py-2.5 text-[12.5px] font-semibold text-mist-400 transition hover:text-mist-100"
              >
                Отмена
              </button>
            </div>
...
export default function AdminPanel({
  ...
  const resetConfirm = useConfirmation();
  const resetPositionsConfirm = useConfirmation();

  const selected = selectedId ? findAny(data, selectedId) : null;

  useEffect(() => {
    if (selectedId && !findAny(data, selectedId)) {
      setSelectedId(null);
    }
  }, [data, selectedId]);
    ...
    <div className="relative z-10 flex h-full w-full flex-col">
      ...
      <header className="flex shrink-0 flex-wrap items-center gap-x-4 gap-y-2 border-b border-ink-700/60 bg-ink-900/85 px-4 py-3 backdrop-blur-md md:px-6">
        ...
        <nav className="ml-auto flex items-center gap-1 rounded-lg border border-ink-700/60 bg-ink-850 p-1">
          {(["editor", "import"] as const).map((tabKey) => (
            <button
              key={tabKey}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`rounded-lg border px-3 py-1.5 text-[11.5px] font-semibold transition-all ${
              resetPositionsConfirm.confirming
                ? "border-ember/70 bg-ember/15 text-ember"
                : "border-ink-700/70 text-mist-500 hover:border-mist-500/60 hover:text-mist-300"
            }`}
          >
            {resetPositionsConfirm.confirming ? "Точно?" : "Сбросить позиции"}
          </button>
          <button
            onClick={handleReset}
            className={`rounded-lg border px-3 py-1.5 text-[11.5px] font-semibold transition-all ${
          ...
          >
            {resetConfirm.confirming ? "Точно сбросить?" : "Сбросить данные"}
          </button>
          <button
            onClick={onLogout}
            className="rounded-lg border border-ink-700/70 px-3 py-1.5 text-[11.5px] font-semibold text-mist-500 transition hover:border-mist-500/60 hover:text-mist-300"
          >
            Выйти
          </button>
        </div>
                ...
                {(["all", "company", "value", "root", "support"] as const).map((filterKey) => (
                  <button
                    ...
                    className={`flex-1 rounded-md border px-1 py-1 text-[10.5px] font-semibold transition ${
                      tierFilter === filterKey
                        ? "border-lagoon/50 bg-lagoon/10 text-lagoon"
